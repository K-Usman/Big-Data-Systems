...
import de.ddm.actors.profiling.DataProviderProxy;
...
public class Worker extends AbstractBehavior<Worker.Message> {
	...
	private Worker(ActorContext<Message> context) {
		...
		this.dataProviderProxy = context.spawn(DataProviderProxy.create(), DataProviderProxy.DEFAULT_NAME);
		this.workers = new ArrayList<>(numWorkers);
		for (int id = 0; id < numWorkers; id++)
			this.workers.add(context.spawn(DependencyWorker.create(this.dataProviderProxy), DependencyWorker.DEFAULT_NAME + "_" + id, DispatcherSelector.fromConfig("akka.worker-pool-dispatcher")));
	}
	...
	final ActorRef<DataProviderProxy.Message> dataProviderProxy;
